  ...
  return (
    <div className="space-y-6">
      <header className="space-y-2">
        <h2 className="text-2xl font-bold">Antrenament completat!</h2>
        <p className="text-sm text-muted-foreground">
          {session.workoutTableName ?? 'Antrenament liber'} —{' '}
          {formatDate(session.startedAt)} — Durata: {formatTime(totalDuration)}
        </p>
        <div className="mt-3">
      ...
      <ul className="flex flex-col gap-4">
        {rows.map((row) => {
          ...
          const unit = row.measurementTypeSnapshot === 'time' ? 's' : ' rep';
            ...
            <li key={row.id} className="flex flex-col gap-2.5 p-4 bg-muted/15 rounded-xl border border-border/20">
              <div className="flex items-center justify-between gap-2 text-sm">
                ...
                <span className="text-muted-foreground/70 text-xs">
                  {setsDone}/{plannedSets} serii — {actual}/{plannedSets * plannedTarget}
                  {unit}
                </span>
              </div>
